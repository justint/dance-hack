...
DEFINE_string(camera_resolution,        "640x360",     "Size of the camera frames to ask for.");
DEFINE_double(camera_fps,               30,           "Frame rate for the webcam (only used when saving video from webcam). Set this value to the"
...
DEFINE_string(net_resolution,           "-1x160",       "Multiples of 16. If it is increased, the accuracy potentially increases. If it is"
...
public:
	...
	typedef std::queue<op::Array<float>> KeypointQueue;
	KeypointQueue poseQueue;
	...
	void printKeypoints(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		unsigned const int QUEUE_SIZE = 15;

		if (datumsPtr != nullptr && !datumsPtr->empty() && datumsPtr->at(0).poseKeypoints.getSize(0) != 0)
		{

			// Acquire pose keypoints
			const auto poseKeypoints = datumsPtr->at(0).poseKeypoints.clone();
			// Push pose keypoints into our queue
			poseQueue.push(poseKeypoints);

			// If our queue is at the right size for processing, push all pose keypoints off and trace the movement for each point
			if (poseQueue.size() > QUEUE_SIZE) {

				std::vector<op::Array<float>> emptiedQueue;
				while (!poseQueue.empty()) {
					emptiedQueue.push_back(poseQueue.front());
					poseQueue.pop();
				}

				// 18 body parts, X & Y
				float averageOfKeypoints[18][2] = { { 0 } };

				// calculate the sum of each bodypart's x and y
				for (int frame = 0; frame < QUEUE_SIZE; frame++) {
					for (int bodyPart = 0; bodyPart < 18; bodyPart++)
					{
						// x
						averageOfKeypoints[bodyPart][0] += emptiedQueue[frame][{0, bodyPart, 0}];
						// y
						averageOfKeypoints[bodyPart][1] += emptiedQueue[frame][{0, bodyPart, 1}];
					}
				}

				// calculate the mean of each bodypart's x and y
				for (int bodyPart = 0; bodyPart < 18; bodyPart++) {
					// x
					averageOfKeypoints[bodyPart][0] = averageOfKeypoints[bodyPart][0] / QUEUE_SIZE;
					// y
					averageOfKeypoints[bodyPart][1] = averageOfKeypoints[bodyPart][1] / QUEUE_SIZE;
				}

				// step 2 of standard deviation
				float minusMeanSquaredKeypoints[18][2] = { {0} };

				// for each bodypart x and y, subtract the mean and square the result
				for (int frame = 0; frame < QUEUE_SIZE; frame++) {
					for (int bodyPart = 0; bodyPart < 18; bodyPart++)
					{
						// x
						minusMeanSquaredKeypoints[bodyPart][0] = (poseKeypoints[{0, bodyPart, 0}] - averageOfKeypoints[bodyPart][0]) * (poseKeypoints[{0, bodyPart, 0}] - averageOfKeypoints[bodyPart][0]);
						// y
						minusMeanSquaredKeypoints[bodyPart][1] = (poseKeypoints[{0, bodyPart, 1}] - averageOfKeypoints[bodyPart][1]) * (poseKeypoints[{0, bodyPart, 1}] - averageOfKeypoints[bodyPart][1]);
					}
				}

				float standardDeviation[18][2] = { { 0 } };

				// calculate the mean of the minusMeanSquaredKeypoints
				for (int bodyPart = 0; bodyPart < 18; bodyPart++) {
					// x
					standardDeviation[bodyPart][0] = minusMeanSquaredKeypoints[bodyPart][0] / QUEUE_SIZE;
					standardDeviation[bodyPart][0] = sqrt(standardDeviation[bodyPart][0]);
					// y
					standardDeviation[bodyPart][1] = minusMeanSquaredKeypoints[bodyPart][1] / QUEUE_SIZE;
					standardDeviation[bodyPart][1] = sqrt(standardDeviation[bodyPart][1]);
				}

				op::log("Standard deviation of pt4 x: " + std::to_string(standardDeviation[4][0]));
				op::log("Standard deviation of pt4 y: " + std::to_string(standardDeviation[4][1]));
				op::log("Standard deviation of pt7 x: " + std::to_string(standardDeviation[7][0]));
				op::log("Standard deviation of pt7 y: " + std::to_string(standardDeviation[7][1]));

				std::string command = "start python playground.py";

				command += " " + std::to_string(standardDeviation[4][0]) + " " + std::to_string(standardDeviation[4][1]) + " " + std::to_string(standardDeviation[7][0]) + " " + std::to_string(standardDeviation[7][1]);
